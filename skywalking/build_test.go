...
import (
	...
	"github.com/paketo-buildpacks/apache-skywalking/skywalking"
)
...
func testBuild(t *testing.T, context spec.G, it spec.S) {
	...
	it("contributes Java agent API <= 0.6", func() {
		ctx.Plan.Entries = append(ctx.Plan.Entries, libcnb.BuildpackPlanEntry{Name: "skywalking-java"})
		...
		result, err := skywalking.Build{}.Build(ctx)
		Expect(err).NotTo(HaveOccurred())

		Expect(result.Layers).To(HaveLen(2))
		Expect(result.Layers[0].Name()).To(Equal("skywalking-java"))
		...
		Expect(result.BOM.Entries[0].Name).To(Equal("skywalking-java"))
			...
			"dependencies": []map[string]interface{}{
				{
					...
					"version": "1.1.1",
					"stacks":  []interface{}{"test-stack-id"},
					"cpes":    []interface{}{"cpe:2.3:a:apache:skywalking:1.1.1:*:*:*:*:*:*:*"},
					"purl":    "pkg:generic/apache-skywalking@1.1.1?arch=amd64",
				},
			},
		}
