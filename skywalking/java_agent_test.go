...
func testJavaAgent(t *testing.T, context spec.G, it spec.S) {
	...
	it("contributes Java agent", func() {
		dep := libpak.BuildpackDependency{
			URI:    "https://localhost/stub-apache-skywalking-agent.tar.gz",
			SHA256: "7fe6a1cd47bfc14ef1a453d618498360e582a7a8d19928adf0df1e66a1696751",
		}
		dc := libpak.DependencyCache{CachePath: "testdata"}

		j, _ := skywalking.NewJavaAgent(dep, dc)
		...
		Expect(layer.LaunchEnvironment["JAVA_TOOL_OPTIONS.append"]).To(Equal(fmt.Sprintf("-javaagent:%s",
			filepath.Join(layer.Path, "skywalking-agent.jar"))))
	})
}
